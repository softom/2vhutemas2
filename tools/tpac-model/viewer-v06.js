...
try{
 ...
 controls=new T.OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.target.set(0,24,0);controls.minDistance=8;controls.maxDistance=450;controls.maxPolarAngle=Math.PI*.495;
...
function view(name){
 const views={home:[165,110,125],front:[-115,55,-150],top:[0,235,.1],core:[88,8,-45],ground:[52,6,-69],entry:[7,3.2,-34]};
 camera.position.set(...views[name]);camera.zoom=Math.min(1,camera.aspect/.95);camera.updateProjectionMatrix();if(controls){controls.target.set(...({core:[39,9,-10],ground:[0,3.8,-4],entry:[-1,3,0]}[name]||[0,20,0]));controls.update();}
 document.querySelectorAll('nav button').forEach(b=>b.setAttribute('aria-pressed',String(b.id===name)));
 if(fallback)document.getElementById('poster').src='review/'+({home:'02_grand_theater',front:'01_globe_street',top:'03_roof_site',core:'04_circulation',ground:'05_open_ground',entry:'06_recessed_entry'}[name])+'.png';
}
view('home');
for(const name of ['home','front','top','core','ground','entry'])document.getElementById(name).onclick=()=>view(name);
...
new T.GLTFLoader().load('tpac-v06.glb',gltf=>{
 ...
 if(!fallback)statusEl.textContent='1,82 МБ · '+meshes+' объектов · '+triangles.toLocaleString('ru-RU')+' треугольников';
