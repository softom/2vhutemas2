...
await page.goto('http://127.0.0.1:8766/prototype-v01/viewer.html?software=1');
...
const bytes=await page.evaluate(async()=>Array.from(new Uint8Array(await exportGLB())));fs.writeFileSync(path.join(__dirname,'tpac-study-v01.glb'),Buffer.from(bytes));
