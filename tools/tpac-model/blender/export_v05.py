...
out=Path(r'E:/Dropbox/Приложения/remotely-save/LearnANDThink/Зрелищные здания/img/taipei-performing-arts-center/model/blender-v05')
master=bpy.context.scene
assert master.name=='TPAC_work_v05'
def stats(scene):
...
bpy.data.libraries.write(str(out/'tpac-master-v05.blend'),{master},fake_user=True,compress=True)
export_scene=bpy.data.scenes.new('TPAC_export_v05')
...
for ob in list(master.objects):
    ...
    n=ob.name
    category=('Globe' if 'Globe' in n else 'Grand Theater' if n=='Grand Theater' or 'Hall' in n or 'Ribbon mullion' in n or 'Recessed ribbon' in n else 'Blue Box' if 'Blue Box' in n or 'Terrace' in n or 'Roof circular' in n else 'Service' if any(t in n for t in ['Rear','tower','Loading','ramp','Public loop','Public circulation']) else 'Building' if any(t in n for t in ['glass','glazing','Facade','facade','Glazed','Roof','lobby','Podium','Entry','Entrance','Canopy','Door']) else 'Site')
...
report={'master':master_metrics,'export':export_metrics,'roundtrip':import_metrics,'bytes':(out/'tpac-v05.glb').stat().st_size,'materials':len({m.name for o in check.objects if o.type=='MESH' for m in o.data.materials if m}),'roundtrip_passed':True,'cameras_calibrated':False,'independent_backup_verified':False}
