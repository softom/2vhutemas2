...
const root='E:/Dropbox/Приложения/remotely-save/LearnANDThink/Зрелищные здания/img/taipei-performing-arts-center/model/blender-v05';
...
await page.goto('http://127.0.0.1:8766/blender-v05/viewer.html');await page.waitForFunction(()=>window.tpac.loaded||window.tpac.error);
...
for(const id of ['front','top','home'])await page.locator('#'+id).click();
...
await page.goto('http://127.0.0.1:8766/blender-v05/review/index.html');
...
if(metrics.fallback||errors.length||mobileOverflow||brokenImages.length||metrics.error||metrics.meshes!==34||metrics.triangles!==40439)process.exitCode=1;
